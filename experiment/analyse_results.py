...
def bin_accuracies_from_probs_tuples(probs_tuples: Iterable[Tuple[float, int]],
    ...
    binned_conf_predclass = [
        list(
            filter(
                lambda x: True if (
                    x[0] > lower and x[0] <= upper) else False,
                probs_tuples)) for lower,
        upper in zip(
            lower_bounds,
            upper_bounds)]
    ...
    for binned in binned_conf_predclass:
        ...
        # the 0-th index
        binned_classes.append(
            [np.where(pair[1] == 0)[0].shape[0] for pair in binned])
    ...
    # in that bin
    accuracies = [sum(binned) / len(binned) if len(binned) >
                  0 else 0 for binned in binned_classes]
    if get_confidence_data:
        ...
        confidences = [sum(confs) / len(confs) if len(confs)
                       > 0 else 0 for confs in binned_conf]
