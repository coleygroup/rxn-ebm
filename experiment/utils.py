...
def load_model_opt_and_stats(saved_stats_filename: Union[str, bytes, os.PathLike], 
                        ...
                        model_name: str='FeedforwardFingerprint', optimizer_name: str='Adam'):
    '''
    Parameters
    ----------
    ...
    saved_stats = torch.load(Path(checkpoint_folder) / Path(saved_stats_filename), 
              map_location=torch.device(curr_device))

    try: 
        checkpoint_filename = saved_stats_filename[:-9] + f'checkpoint_{str(saved_stats["best_epoch"]).zfill(4)}.pth.tar'  
        checkpoint = torch.load(Path(checkpoint_folder) / Path(checkpoint_filename),
              map_location=torch.device(curr_device))
        ...
        if model_name == 'FeedforwardFingerprint' or model_name == 'FeedforwardEBM':
            saved_model = FeedforwardFingerprint(**saved_stats['model_args'])
        else:
        ...
        saved_optimizer = saved_stats['train_args']['optimizer'](saved_model.parameters(), lr=saved_stats['train_args']['learning_rate'])
