...
class Experiment():
    ...
    def _init_dataloaders(self, precomp_file_prefix: str, onthefly: bool, lookup_dict_filename: str, mol_fps_filename: str,
        ...
        train_dataset = ReactionDataset(
            input_dim, precomp_filenames['train'], rxn_smis_filenames['train'], onthefly, augmented_data)
        ...
        val_dataset = ReactionDataset(
           input_dim, precomp_filenames['valid'], rxn_smis_filenames['valid'], onthefly, augmented_data)
        ...
        test_dataset = ReactionDataset(
            input_dim, precomp_filenames['test'], rxn_smis_filenames['test'], onthefly, augmented_data)
    ...
    def test(self, saved_stats: Optional[dict]=None):
        ...
        if len(self.stats.keys()) <= 2:
            raise RuntimeError('self.stats only has 2 keys or less. If loading checkpoint, you need to provide load_stats!')
    ...
    def get_scores_and_loss(self, dataset_name: Optional[str]='test',
        ...
        if dataloader is not None:
            if dataset_len is None:
                raise ValueError('Please provide the size of the custom dataset')
            dataset_name = 'custom'
