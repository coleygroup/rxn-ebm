    ...
    for split in splits:
        ...
        for i, rxn_smi in enumerate(tqdm(rxn_smi_split[:20])):
            ...
            for j, mut_prod_smi in enumerate(crem.mutate_mol(
                    prod_mol, db_name=frag_db, radius=radius, max_size=max_size, return_mol=False, ncores=ncores)):
...
if __name__ == '__main__':
    # mol_smi_from_pickle('50k_mol_smis', '50k_mol_smis') # then run
    # crem_create_frag_db.sh from command prompt
