...
class ReactionDataset(Dataset):
    ...
    def __init__(self, base_path, key, trainargs, mode,
        ...
        if mode == 'bit_corruption':
            ...
            self.num_bits = trainargs['num_bits']
        else:
            ...
            'You did not provide num_neg_prod and num_neg_rct!'
    ...
    def cosine_sample_negative(self, raw_fp, num_rcts):
        ...
        '''
        rct_idx = random.choice(np.arange(num_rcts)) 
        # might be faster to sample rct_idx's for entire batch (have to design custom collate_fn)
        
        rct_prod_sparse = sparse.csr_matrix(raw_fp.copy()[[rct_idx, -1]], dtype='int8')
        nn_rct_indices, nn_prod_indices = self.clusterindex.search(rct_prod_sparse, k=max(self.num_neg_prod, self.num_neg_rct)+1, 
