...
def move_reagents(mol_prod: Mol, reactants: List[Mol], 
...
def clean_rxn_smis_from_csv(path_to_rxn_smis: Union[str, bytes, os.PathLike], 
                        lines_to_skip: int = 1, dataset: str = '50k', split_mode: str = 'single',
    ...
    set_clean_list : Set[str]
        set of cleaned reaction SMILES strings
        this will be used if remove_dup_rxns is True in clean_rxn_smis_all_splits()
    ...
    with open(path_to_rxn_smis, 'r') as csv_file:
        ...
        for line in range(lines_to_skip): 
            header = next(reader) # skip first row of csv file (for USPTO_STEREO, 3 lines to be skipped)
        ...
        for i, row in enumerate(tqdm(reader, total=total_lines)):
            if dataset == '50k':
                ...
                all_rcts_smi, all_reag_smi, prods_smi = rxn_smiles.split('>')
            ...
            for prod_smi in prods_smi.split('.'):
                ...
                # check if all have atom mapping, but for rxn-ebm, this is not important
        ...
        print('bad mapping: ', bad_mapping, bad_mapping_idxs)
        print('bad prod: ', bad_prod, bad_prod_idxs)
        print('too small: ', too_small, too_small_idxs)
        print('missing map: ', missing_map, missing_map_idxs)
        print('raw rxn extracted: ', raw_num, '\n')
        return clean_list, set_clean_list

def clean_rxn_smis_all_splits(input_file_prefix: str, output_file_prefix: str='50k_clean_rxnsmi_noreagent', 
                             input_root: Optional[Union[str, bytes, os.PathLike]] = None, 
                             output_root: Optional[Union[str, bytes, os.PathLike]] = None, 
                             dataset: str = '50k', lines_to_skip: int = 1, 
                             keep_reagents: bool = False, remove_dup_rxns: bool = True,
                             remove_rct_mapping: bool = True, remove_all_mapping: bool = True,
    ...
    input_file_prefix : str 
        file prefix of the original raw, possibly unclean, reaction SMILES csv file 
    ...
    for split in cleaned_rxn_smis.keys():
        ...
        with Pool(max_workers=num_workers) as client:
            future = client.submit(
                ...
                dataset=dataset, lines_to_skip=lines_to_skip, keep_reagents=keep_reagents, 
                remove_rct_mapping=remove_rct_mapping, remove_all_mapping=remove_all_mapping) 
            if remove_dup_rxns: 
                cleaned_rxn_smis[split] = future.result()[1] # [1] is set_clean_list
            else:
                cleaned_rxn_smis[split] = future.result()[0]

        with open(output_root / f'{output_file_prefix}_{split}.pickle', 'wb') as handle:
            pickle.dump(cleaned_rxn_smis[split], handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def get_uniq_mol_smis_all_splits(rxn_smi_file_prefix: str='50k_clean_rxnsmi_noreagent', 
    ...
    uniq_mol_smis = set()
    # loop through all 3 splits, and collect all unique reactants & products (not reagents!)
    for split in cleaned_rxn_smis.keys(): 
...
if __name__ == '__main__':
    ...
    clean_rxn_smis_all_splits(raw_data_file_prefix, lines_to_skip=1)
    get_uniq_mol_smis_all_splits() 


# code from clean_uspto.py of GLN for splitting USPTO_FULL into 80/10/10
#     seed = 19260817
#     np.random.seed(seed)
#     random.seed(seed)
''' 
