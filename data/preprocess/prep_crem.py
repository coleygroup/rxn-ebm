...
def gen_crem_negs(num_neg: int,
                  max_size: int,
                  radius: int,
                  frag_db_filename: Union[str,
                                 bytes,
                                 os.PathLike] = 'replacements02_sa2.db',
                  rxn_smi_file_prefix: Union[str,
                                             bytes,
                                             os.PathLike] = '50k_clean_rxnsmi_noreagent',
                  dataset_name: Optional[str] = '50k',
                  root: Optional[Union[str,
                                       bytes,
                                       os.PathLike]] = None,
                  phases: Optional[Union[str,
                                         List[str]]] = None,
    ...
    for phase in phases:
        ...
        for i, rxn_smi in enumerate(tqdm(rxn_smi_phase[:20])):
            ...
            this_rxn_mut = [] 
            j = 0
            for j, mut_prod_smi in enumerate(crem.mutate_mol(
                    prod_mol, db_name=str(root / frag_db_filename), radius=radius, max_size=max_size, return_mol=False, ncores=ncores)):
            ...
            if i % 2500 == 0: # checkpoint 
                with open(root / f'{dataset_name}_neg{num_neg}_rad{radius}_maxsize{max_size}_{phase}_mutprodsmis.pickle', 'wb') as handle:
                    pickle.dump(all_mut_prod_smi, handle, pickle.HIGHEST_PROTOCOL)
                with open(root / f'{dataset_name}_neg{num_neg}_rad{radius}_maxsize{max_size}_{phase}_insufficient.pickle', 'wb') as handle:
...
if __name__ == '__main__':
    ...
    gen_crem_negs(num_neg=150, max_size=3, radius=2,
