...
class Retrosim:
    ...
    def propose_all(self) -> None:
        ...
        and proposes precursors for them based on self.max_templates and self.topk
        ...
        if self.parallelize: 
            ...
            output_dicts = Parallel(n_jobs=num_workers, verbose=5)(
                    delayed(self.propose_one_helper)(
                            prod_smi, results, self.topk, self.max_templates
                        ) 
            ...
            for i, prod_smi in enumerate(tqdm(prod_smiles)):
                ...
                all_proposed_smiles[prod_smi] = self.propose_one(prod_smi, self.topk, self.max_templates)
        ...
        with open(
                self.output_folder / 
                f'all_proposed_smiles_{self.topk}maxtest_{self.max_templates}maxprec.pickle', 'wb'
            ) as handle:
            pickle.dump(all_proposed_smiles, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def compile_into_csv(self):
        ...
        if self.all_proposed_smiles is None:
            with open(
                    self.output_folder / 
                    f'all_proposed_smiles_{self.topk}maxtest_{self.max_templates}maxprec.pickle', 'rb'
                ) as handle:
        ...
        for phase in self.phases:
            ...
            phase_dataframe.to_csv(
                self.output_folder / 
                f'retrosim_{self.topk}maxtest_{self.max_templates}maxprec_{phase}.csv'
            )
