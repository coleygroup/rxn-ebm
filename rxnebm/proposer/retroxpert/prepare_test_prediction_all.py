...
for phase in ['train', 'valid']:
    pred_results = 'logs/{}_result_mol_{}_{}_{}.txt'.format(phase, args.dataset, args.typed, args.seed) # add seed
    with open(pred_results) as f:
        pred_results = f.readlines()

    bond_pred_results = 'logs/{}_disconnection_{}_{}_{}.txt'.format(phase, args.dataset, args.typed, args.seed)
    with open(bond_pred_results) as f:
        bond_pred_results = f.readlines()

    dataset_dir = 'data/{}/{}'.format(args.dataset, phase)
    ...
    with open(f'opennmt_data/src-{phase}.txt') as f:
        srcs = f.readlines()
    ...
    src_test_prediction = os.path.join(savedir, f'src-{phase}-prediction.txt')
