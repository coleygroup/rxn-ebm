...
class Augmentor(ABC):
    ...
    def get_one_sample(self, rxn_smi: str, **kwargs
        ...
        """ Generate num_neg * negative_rxn_smi from a given rxn_smi (if return_type == "smi")
            or num_neg * negative_rxn_fps (if return_type == "fp")
        """
    ...
    # only declares method that you want to force any sub-class to implement
    # @abstractmethod
    ...
    def get_one_sample(self, rxn_smi: str, **kwargs
        ...
        if self.return_type == "smi":
            ...
            for mut_prod_smi in mut_prod_smis:
                neg_rxn_smi = f"{rcts_smi}>>{mut_prod_smi}"
                neg_rxn_smis.append(neg_rxn_smi)

            while len(neg_rxn_smis) < self.num_neg:
            ...
            # pad with zero-vectors whenever insufficient negs generated from CReM
            if len(neg_rxn_fps) < self.num_neg:
                dummy_fp = np.zeros((1, self.fp_size), dtype=self.dtype)
                neg_rxn_fps.extend([dummy_fp] * (self.num_neg - len(neg_rxn_fps)))

            return neg_rxn_fps

        else:
            raise ValueError(f"return_type {self.return_type} not supported for Mutate Augmentor!")
    ...
    def get_one_sample(self, rxn_smi: str, **kwargs
        ...
        if self.return_type == "smi":
            ...
            for nn_prod_smi in nn_prod_smis:
                neg_rxn_smi = f"{rcts_smi}>>{nn_prod_smi}"
                neg_rxn_smis.append(neg_rxn_smi)
            ...
            raise ValueError(f"return_type {self.return_type} not supported for Cosine Augmentor!")
...
class Bit(Augmentor):
    ...
    def __init__(
        ...
        super().__init__(num_neg, smi_to_fp_dict, mol_fps, rxn_type, fp_type)
