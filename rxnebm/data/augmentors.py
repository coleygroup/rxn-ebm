...
class Augmentor(ABC):
    ...
    def __init__(self, smi_to_fp_dict: dict, mol_fps: sparse_fp):
        self.smi_to_fp_dict = smi_to_fp_dict
        self.mol_fps = mol_fps

    @abstractmethod
    def get_one_sample(self, rxn_smi: str, num_neg: int, return_type: str, **kwargs
...
class Mutate(Augmentor):
    ...
    def __init__(
        ...
        super().__init__(smi_to_fp_dict, mol_fps)
    ...
    def get_one_sample(self, rxn_smi: str, num_neg: int, return_type: str, **kwargs
        ...
        if return_type == "smi":
            ...
            if len(mut_prod_smis) > num_neg:
                mut_prod_smis = random.sample(mut_prod_smis, num_neg)
            ...
            while len(neg_rxn_smis) < num_neg:
            ...
            fp_size = kwargs.get("fp_size", 4096)
            dtype = kwargs.get("dtype", "int32")

            if fp_type == "count":
                mol_smi_to_fp = smi_to_fp.mol_smi_to_count_fp
            elif fp_type == "bit":
                mol_smi_to_fp = smi_to_fp.mol_smi_to_bit_fp
            else:
                raise ValueError(f"fp_type {fp_type} not supported for Mutate Augmentor!")
            ...
            mut_prod_fps = []
            for mut_prod_smi in mut_prod_smis:
                prod_fp = mol_smi_to_fp(mut_prod_smi, radius, fp_size, dtype)
                mut_prod_fps.append(prod_fp)

            rcts_fp, _ = rcts_prod_fps_from_rxn_smi(
                rxn_smi, fp_type, self.smi_to_fp_dict, self.mol_fps
            )

            neg_rxn_fps = []
            for mut_prod_fp in mut_prod_fps:
                neg_rxn_fp = make_rxn_fp(rcts_fp, mut_prod_fp, rxn_type)
                neg_rxn_fps.append(neg_rxn_fp)

            # pad with zero-vectors whenever insufficient negs generated from CReM
            dummy_fp = np.zeros((1, fp_size), dtype=dtype)
            while len(neg_rxn_fps) < num_neg:
                neg_rxn_fps.append(dummy_fp)

            return neg_rxn_fps

        else:
            raise ValueError(f"return_type {return_type} not supported for Mutate Augmentor!")
...
class Cosine(Augmentor):
    ...
    def get_one_sample(self, rxn_smi: str, num_neg: int, return_type: str, **kwargs
        ...
        if return_type == "smi":
            fp_type = kwargs.get("fp_type", "count")
            num_threads = kwargs.get("num_threads", 4)

            rcts_fp, prod_fp = rcts_prod_fps_from_rxn_smi(
                rxn_smi, fp_type, self.smi_to_fp_dict, self.mol_fps)
            nn_prod_idxs = self.get_idx(prod_fp=prod_fp, rxn_smi=rxn_smi, num_neg=num_neg, num_threads=num_threads)
            nn_prod_smis = [self.fp_to_smi_dict[idx] for idx in nn_prod_idxs[:num_neg]]
            ...
            rxn_type = kwargs.get("rxn_type", "diff")
            fp_type = kwargs.get("fp_type", "count")
            num_threads = kwargs.get("num_threads", 4)
            ...
            rcts_fp, prod_fp = rcts_prod_fps_from_rxn_smi(
                rxn_smi, fp_type, self.smi_to_fp_dict, self.mol_fps)
            nn_prod_idxs = self.get_idx(prod_fp=prod_fp, rxn_smi=rxn_smi, num_neg=num_neg, num_threads=num_threads)
            nn_prod_fps = [self.mol_fps[idx] for idx in nn_prod_idxs[:num_neg]]

            neg_rxn_fps = []
            for nn_prod_fp in nn_prod_fps:
                neg_rxn_fp = make_rxn_fp(rcts_fp, nn_prod_fp, rxn_type)
                neg_rxn_fps.append(neg_rxn_fp)
            return neg_rxn_fps

        else:
            raise ValueError(f"return_type {return_type} not supported for Cosine Augmentor!")

    def get_idx(self, prod_fp: sparse_fp, rxn_smi: str, num_neg: int, num_threads: int) -> List[int]:
        nn_prod_idxs_with_dist = self.search_index.knnQueryBatch(
            prod_fp, k=num_neg + 3, num_threads=num_threads)
        ...
        for prod_idx in nn_prod_idxs[1:]:           # 0th element = orig prod; we don't want it as negative
            ...
            if new_prod_smi not in all_rct_smis:    # prod == one of the rcts, do not use this prod!
    ...
    def get_one_sample(self, rxn_smi: str, num_neg: int, return_type: str, **kwargs
        ...
        if return_type == "smi":
            ...
            for i in range(num_neg):
                ...
                neg_rxn_fp = make_rxn_fp(rcts_fp, rdm_prod_fp, rxn_type)
                neg_rxn_fps.append(neg_rxn_fp)
            return neg_rxn_fps

        else:
            raise ValueError(f"return_type {return_type} not supported for Random Augmentor!")
...
class Bit(Augmentor):
    ...
    def get_one_sample(self, rxn_smi: str, num_neg: int, return_type: str, **kwargs
        ...
        if return_type == "fp":
            ...
            increment_bits = kwargs["increment_bits"] or 1

            rcts_fp, prod_fp = rcts_prod_fps_from_rxn_smi(
                rxn_smi, fp_type, self.smi_to_fp_dict, self.mol_fps)
            pos_rxn_fp = make_rxn_fp(rcts_fp, prod_fp, rxn_type)

            neg_rxn_fps = []
            for i in range(num_neg):
                neg_rxn_fp = pos_rxn_fp.copy()
                rdm_bit_idxs = random.sample(range(pos_rxn_fp.shape[-1]), k=num_bits)
                for bit_idx in rdm_bit_idxs:
                    if fp_type == "count":
                        neg_rxn_fp[0, bit_idx] += increment_bits
                    elif fp_type == "bit":
            ...
            raise ValueError(f"return_type {return_type} not supported for Bit Augmentor!")
