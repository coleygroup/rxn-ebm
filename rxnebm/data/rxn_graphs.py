...
from rxnebm.data.chem_utils import BOND_TYPES, get_sub_mol
...
class RxnElement:
    ...
    def update_atom_scope(self, offset: int) -> Union[List, Tuple]:
        ...
        if isinstance(self.atom_scope, list):
            self.atom_scope = [(st + offset, le) for (st, le) in self.atom_scope]
        else:
            st, le = self.atom_scope
            self.atom_scope = (st + offset, le)
        return self.atom_scope
    ...
    def update_bond_scope(self, offset: int) -> Union[List, Tuple]:
        ...
        if isinstance(self.bond_scope, list):
            self.bond_scope = [(st + offset, le) for (st, le) in self.bond_scope]
        else:
            st, le = self.bond_scope
            self.bond_scope = (st + offset, le)
        return self.bond_scope
