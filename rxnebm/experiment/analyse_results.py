...
def bin_accuracies_from_probs_tuples(
    probs_tuples: Iterable[Tuple[float, int]],
    lower_bounds: Iterable[float] = np.linspace(0.0, 0.95, 20),
    upper_bounds: Iterable[float] = np.linspace(0.05, 1.0, 20),
    ...
    """
    # puts (probability, predicted index) tuples into bins defined by lower &
    # upper bounds
    binned_conf_predclass = [
        list(
            filter(
                lambda x: True if (x[0] > lower and x[0] <= upper) else False,
                probs_tuples,
            )
        )
    ...
    for binned in binned_conf_predclass:
        ...
        # model was correct iff pair[1] == 0 since pos rxn is always and only
        # the 0-th index
    ...
    # for plotting purposes, assigns default accuracy of 0 if no members exist
    # in that bin
    accuracies = [
        sum(binned) / len(binned) if len(binned) > 0 else 0 for binned in binned_classes
    ]
    if get_confidence_data:
        ...
        confidences = [
            sum(confs) / len(confs) if len(confs) > 0 else 0 for confs in binned_conf
        ]
