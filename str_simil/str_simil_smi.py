...
def simulate_root(): # just root
    with open(Path(__file__).resolve().parents[1] / 'rxnebm/data/cleaned_data/50k_clean_rxnsmi_noreagent_canon_train.pickle', 'rb') as f:
        train = pickle.load(f)
    ...
    with open(Path(__file__).resolve().parents[1] / f'rxnebm/data/string_similarity/{expt_name}.pickle', 'wb') as f:
        pickle.dump(data, f)
