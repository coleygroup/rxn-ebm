...
''' TODO: add support for return_type = ['SMILES', 'Graphs'] for transformer/GNNs
'''
...
def main(args):
    ...
    2. Process all proposed SMILES into fingerprints, also takes some time: about 40 min for train, 5 min each for valid & test. 
    '''
    # retrieve args 
            ...
            if phase == 'train':
                ...
                for rxn in tqdm(processed_rxn_smis, desc='Generating rxn_fps...'):
                    ...
                    for rxn_smi in rxn_smis:
                        rcts_fp, prod_fp = augmentors.rcts_prod_fps_from_rxn_smi(rxn_smi, 'count', mol_smi_to_fp, count_mol_fps)
                        rxn_fp = augmentors.make_rxn_fp(rcts_fp, prod_fp, 'diff')
